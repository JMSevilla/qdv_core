...
const InputBase = /*#__PURE__*/React.forwardRef(function InputBase(props, ref) {
  ...
  const handleInputRefWarning = React.useCallback(instance => {
    if (process.env.NODE_ENV !== 'production') {
      if (instance && instance.nodeName !== 'INPUT' && !instance.focus) {
        console.error(['Qodev-UI: You have provided a `inputComponent` to the input component', 'that does not correctly handle the `inputRef` prop.', 'Make sure the `inputRef` prop is called with a HTMLInputElement.'].join('\n'));
      }
    }
  }, []);
  ...
  const handleChange = (event, ...args) => {
    if (!isControlled) {
      ...
      if (element == null) {
        throw new Error(process.env.NODE_ENV !== "production" ? `Qodev-UI: Expected valid input target. Did you use a custom \`inputComponent\` and forget to forward refs? See https://mui.com/r/input-component-ref-interface for more info.` : _formatQdvErrorMessage(1));
      }

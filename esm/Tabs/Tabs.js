...
var Tabs = /*#__PURE__*/React.forwardRef(function Tabs(props, ref) {
  ...
  if (process.env.NODE_ENV !== 'production') {
    if (centered && scrollable) {
      console.error('Qodev-UI: You can not use the `centered={true}` and `variant="scrollable"` properties ' + 'at the same time on a `Tabs` component.');
    }
  }
  ...
  var getTabsMeta = function getTabsMeta() {
    ...
    if (tabsNode && value !== false) {
      ...
      if (_children.length > 0) {
        ...
        if (process.env.NODE_ENV !== 'production') {
          if (!tab) {
            console.error(["Qodev-UI: The value provided to the Tabs component is invalid.", "None of the Tabs' children match with `".concat(value, "`."), valueToIndex.keys ? "You can provide one of the following values: ".concat(Array.from(valueToIndex.keys()).join(', '), ".") : null].join('\n'));
          }
        }
  ...
  var children = React.Children.map(childrenProp, function (child) {
    ...
    if (process.env.NODE_ENV !== 'production') {
      if (isFragment(child)) {
        console.error(["Qodev-UI: The Tabs component doesn't accept a Fragment as a child.", 'Consider providing an array instead.'].join('\n'));
      }
    }
